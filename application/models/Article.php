...
class Article extends CI_Model {
    ...
    public function save() {
        ...
        if ($this->is_found) { //mise à jour BDD
            $this->db->where('id', $this->_id)->update('article', $data);
        } else {
            $data['date'] = date('Y-m-d H:i:s'); //date déconseillé en php 
            $this->db->insert('article', $data); //insertion en BDD
            $id = $this->db->insert_id();
            $this->load($id, TRUE); //on recharge pour s'assurer que l'insertion à réussi (attention aux ressources) 
        }
    }
    ...
    protected function set_property_title($title) {
        $alias = url_title($title, 'undescore', TRUE); //url_title fournit oas le helper URL 
        $this->_title = $title;
        $this->_alias = $alias;
    }
